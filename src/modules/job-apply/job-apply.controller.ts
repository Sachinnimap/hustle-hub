...
export class JobApplyController {
  ...
  async findAll(@Query() query: PaginationDto) {
    return new Response(
      200,
      'list of jobs applied by candidates',
      await this.jobApplyService.findAll(query),
    );
  }
  ...
  async create(@Body() body: CreateJobApplyDto, @Request() req) {
    return new Response(
      ...
      await this.jobApplyService.create(body, req),
    );
  }
  ...
  async findAllAppliedByMe(@Query() query: PaginationDto, @Request() req) {
    return new Response(
      200,
      'list of jobs applied by you',
      await this.jobApplyService.findAllAppliedByMe(query, req),
    );
  }
  ...
  async findAllJobsByRecruiter(@Query() query: PaginationDto, @Request() req) {
    return new Response(
      200,
      'list of candidate details who applied your job posts',
      await this.jobApplyService.findAllJobsByRecruiter(query, req),
    );
  }
}
