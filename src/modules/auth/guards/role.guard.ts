...
export class RoleGuard implements CanActivate{
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        try{
            ...
            const permission = `select * from permission where base_url='${basePath}' and path ='${path}'`;

    const checkPermission = await this.sequelize.query(permission, {type : QueryTypes.SELECT })
    console.log("checkPermission",checkPermission)
              if(checkPermission.length == 0) throw  new BusinessException("Invalid Route")
                const getRolePermission = `select * from role_permission
where role_id=${request.user.roleId} and permission_id = ${checkPermission[0]['id']}`;

    const checkRolePermission = await this.sequelize.query(getRolePermission, {type : QueryTypes.SELECT })
                if(!checkRolePermission) throw new BusinessException("Access denied!")
      return true
        }
        catch(error){
