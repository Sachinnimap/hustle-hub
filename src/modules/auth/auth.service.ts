...
export class AuthService {
  constructor(
    ...
    private resetPasswordModel: typeof ResetPassword,
  ) {}
  ...
  async register(registerDto: RegisterDto) {
    ...
    const token = await this.createToken({
      userId: userData.id,
      roleId: userData.getDataValue('roleId') ?? 3,
    });
  ...
  async logout(req: any) {
    ...
    if (!user) throw new BusinessException('Could not logout');
    
    let getToken = user.toJSON().token;
    await user?.update({ token: null });
    await this.cacheService.del(getToken);
    return null;
  }
  ...
  async resetPassword(body: ResetDto) {
    ...
    const checkEmail = await this.resetPasswordModel.findOne({
      where: { email },
    });
    if (checkEmail)
      await this.resetPasswordModel.update({ otp }, { where: { email } });
    else await this.resetPasswordModel.create({ email, otp });

    this.mailService.appliedJobMail(
      email,
      `Reset OTP ${otp}`,
      `Your OTP is ${otp}`,
    );
  ...
  async varifyResetPassword(body: ResetPasswordDto) {
    ...
    const userData = await this.resetPasswordModel.findOne({
      where: { email },
      raw: true,
    });
  ...
  async findUser(id:any,token:string){
    return await this.userModel.findOne({where : {id,token}})
  }
}
