...
export class AuthController{
  ...
  async delete(@Param('id') id: number, @Request() req){
    return new Response(201,'Deleted successfully',await this.authService.delete(+id,req))
  }

   @Get('/candidates')
  async getAllCandidates( @Query() query :PaginationDto, @Request() req){
    return new Response(200,'fetched candidates successfully',await this.authService.getAllCandidates(query,req))
  }

    @Get('/recruiters')
  async getAllRecruiters( @Query() query :PaginationDto, @Request() req){
    return new Response(200,'fetched recruiters successfully',await this.authService.getAllRecruiters(query,req))
  }
}
